#!/usr/bin/env python 
"""
Modified by Tiago Rorke 2017, MILL - Makers in Little Lisbon, mill.pt
...
def biarc(sp1, sp2, z1, z2, depth=0):
    ...
    if TE.mag() < straight_distance_tolerance:
        TE = -(TS+v).unit()
        # r = TS.mag()/v.mag()*2
        # was giving divide by 0 errors, so here is a quick hack >
        hack = v.mag()*2
        if hack!=0 :
            r = TS.mag()/hack
        else :
...
class laser_gcode(inkex.Effect):
    ...
    def export_gcode(self,gcode):
        ...
        f.write(self.options.laser_off_command + " S" + str(int(self.options.laser_off_power)) + "\n" + self.header + "G1 F" + self.options.travel_speed + "\n" + gcode + self.footer)
        f.close()
    ...
    def __init__(self):
        ...
        self.OptionParser.add_option("",   "--draw-speed",                      action="store", type="int",             dest="laser_speed",                         default="100",                          help="Speed while drawing (mm/min)")
        self.OptionParser.add_option("",   "--travel-speed",                    action="store", type="string",          dest="travel_speed",                        default="3000",                         help="Speed while not drawing (mm/min)")
        self.OptionParser.add_option("",   "--draw-command",                    action="store", type="string",          dest="laser_command",                       default="M03",                          help="Gcode command to start drawing")
        self.OptionParser.add_option("",   "--draw-S",                          action="store", type="int",             dest="laser_power",                         default="256",                          help="S# is 256 or 10000 for full power")
        self.OptionParser.add_option("",   "--nodraw-command",                  action="store", type="string",          dest="laser_off_command",                   default="M05",                          help="Gcode command to stop drawing")       
        self.OptionParser.add_option("",   "--nodraw-S",                        action="store", type="int",             dest="laser_off_power",                     default="0",                            help="S# is 256 or 10000 for full power")
        ...
        self.OptionParser.add_option("",   "--draw-delay",                      action="store", type="string",          dest="draw_delay",                          default="100",                          help="Delay after start draw command before moving (ms)")
        self.OptionParser.add_option("",   "--nodraw-delay",                    action="store", type="string",          dest="nodraw_delay",                        default="100",                          help="Delay after stop draw command before moving (ms)")
...
################################################################################
    def effect(self) :
        ...
        bd = float(self.options.draw_delay)/1000.0
        ad = float(self.options.nodraw_delay)/1000.0
        self.tools = {
            "name": "Laser Engraver",
            "id": "Laser Engraver",
            ...
            "gcode after path": (self.options.laser_off_command + " S" + str(int(self.options.laser_off_power)) + "\n" + "G4 P" + "{0:.3f}".format(ad) + " \n" + "G1 F" + self.options.travel_speed),
        }
